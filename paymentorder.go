...
type PaymentOrder struct {
	...
	Accounting PaymentOrderAccounting `json:"accounting" api:"required"`
	...
	BatchID string `json:"batch_id" api:"required,nullable" format:"uuid"`
	// The party that will pay the fees for the payment order. See
	// https://docs.moderntreasury.com/payments/docs/charge-bearer to understand the
	// differences between the options.
	ChargeBearer PaymentOrderChargeBearer `json:"charge_bearer" api:"required,nullable"`
	...
	// data.
	CurrentHold PaymentOrderCurrentHold `json:"current_hold" api:"required,nullable"`
	...
	Direction PaymentOrderDirection `json:"direction" api:"required"`
	...
	ForeignExchangeContract string `json:"foreign_exchange_contract" api:"required,nullable"`
	// Indicates the type of FX transfer to initiate, can be either
	// `variable_to_fixed`, `fixed_to_variable`, or `null` if the payment order
	// currency matches the originating account currency.
	ForeignExchangeIndicator PaymentOrderForeignExchangeIndicator `json:"foreign_exchange_indicator" api:"required,nullable"`
	...
	OriginatingSecondaryPartyName string `json:"originating_secondary_party_name" api:"required,nullable"`
	// Either `normal` or `high`. For ACH and EFT payments, `high` represents a
	// same-day ACH or EFT transfer, respectively. For check payments, `high` can mean
	// an overnight check rather than standard mail.
	Priority PaymentOrderPriority `json:"priority" api:"required"`
	// If present, Modern Treasury will not process the payment until after this time.
	// If `process_after` is past the cutoff for `effective_date`, `process_after` will
	// take precedence and `effective_date` will automatically update to reflect the
	// earliest possible sending date after `process_after`. Format is ISO8601
	// timestamp.
	ProcessAfter time.Time `json:"process_after" api:"required,nullable" format:"date-time"`
	...
	ReceivingAccountID   string                           `json:"receiving_account_id" api:"required" format:"uuid"`
	ReceivingAccountType PaymentOrderReceivingAccountType `json:"receiving_account_type" api:"required"`
	// One of `unreconciled`, `tentatively_reconciled` or `reconciled`.
	ReconciliationStatus PaymentOrderReconciliationStatus `json:"reconciliation_status" api:"required"`
	ReferenceNumbers     []PaymentOrderReferenceNumber    `json:"reference_numbers" api:"required"`
	...
	SendRemittanceAdvice bool `json:"send_remittance_advice" api:"required,nullable"`
	...
	Status PaymentOrderStatus `json:"status" api:"required"`
	// An additional layer of classification for the type of payment order you are
	// doing. This field is only used for `ach` payment orders currently. For `ach`
	// payment orders, the `subtype` represents the SEC code. We currently support
	// `CCD`, `PPD`, `IAT`, `CTX`, `WEB`, `CIE`, and `TEL`.
	Subtype PaymentOrderSubtype `json:"subtype" api:"required,nullable"`
	...
	Type PaymentOrderType `json:"type" api:"required"`
	// The account to which the originating of this payment should be attributed to.
	// Can be a `virtual_account` or `internal_account`.
	UltimateOriginatingAccount PaymentOrderUltimateOriginatingAccount `json:"ultimate_originating_account" api:"required,nullable"`
	// The ultimate originating account ID. Can be a `virtual_account` or
	// `internal_account`.
	UltimateOriginatingAccountID   string                                     `json:"ultimate_originating_account_id" api:"required,nullable" format:"uuid"`
	UltimateOriginatingAccountType PaymentOrderUltimateOriginatingAccountType `json:"ultimate_originating_account_type" api:"required,nullable"`
	// Identifier of the ultimate originator of the payment order.
	UltimateOriginatingPartyIdentifier string `json:"ultimate_originating_party_identifier" api:"required,nullable"`
	// Name of the ultimate originator of the payment order.
	UltimateOriginatingPartyName     string    `json:"ultimate_originating_party_name" api:"required,nullable"`
	UltimateReceivingPartyIdentifier string    `json:"ultimate_receiving_party_identifier" api:"required,nullable"`
	UltimateReceivingPartyName       string    `json:"ultimate_receiving_party_name" api:"required,nullable"`
	UpdatedAt                        time.Time `json:"updated_at" api:"required" format:"date-time"`
	// Additional vendor specific fields for this payment. Data must be represented as
	// key-value pairs.
	VendorAttributes interface{} `json:"vendor_attributes" api:"required,nullable"`
	// This field will be populated if a vendor failure occurs. Logic shouldn't be
	// built on its value as it is free-form.
	VendorFailureReason string           `json:"vendor_failure_reason" api:"required,nullable"`
...
type PaymentOrderCurrentHold struct {
	...
	CreatedAt time.Time `json:"created_at" api:"required" format:"date-time"`
	// The type of object
	Object PaymentOrderCurrentHoldObject `json:"object" api:"required"`
	// The status of the hold
	Status PaymentOrderCurrentHoldStatus `json:"status" api:"required"`
	// The ID of the target being held
	TargetID string `json:"target_id" api:"required" format:"uuid"`
	// The type of target being held
	TargetType PaymentOrderCurrentHoldTargetType `json:"target_type" api:"required"`
	UpdatedAt  time.Time                         `json:"updated_at" api:"required" format:"date-time"`
	// This field will be true if this object exists in the live environment or false
	// if it exists in the test environment.
...
type PaymentOrderReferenceNumber struct {
	...
	ReferenceNumberType PaymentOrderReferenceNumbersReferenceNumberType `json:"reference_number_type" api:"required"`
	UpdatedAt           time.Time                                       `json:"updated_at" api:"required" format:"date-time"`
...
const (
	...
	PaymentOrderReferenceNumbersReferenceNumberTypeBlockchainTransactionIndex                     PaymentOrderReferenceNumbersReferenceNumberType = "blockchain_transaction_index"
	...
	PaymentOrderReferenceNumbersReferenceNumberTypeBofaNeftClearingReferenceNumber                PaymentOrderReferenceNumbersReferenceNumberType = "bofa_neft_clearing_reference_number"
	...
	PaymentOrderReferenceNumbersReferenceNumberTypeChipsUid                                       PaymentOrderReferenceNumbersReferenceNumberType = "chips_uid"
	...
	PaymentOrderReferenceNumbersReferenceNumberTypeCrossRiverCardTraceNumber                      PaymentOrderReferenceNumbersReferenceNumberType = "cross_river_card_trace_number"
	...
	PaymentOrderReferenceNumbersReferenceNumberTypeCrossRiverRetrievalReferenceID                 PaymentOrderReferenceNumbersReferenceNumberType = "cross_river_retrieval_reference_id"
	...
	PaymentOrderReferenceNumbersReferenceNumberTypePaxosCryptoWithdrawalID                        PaymentOrderReferenceNumbersReferenceNumberType = "paxos_crypto_withdrawal_id"
	PaymentOrderReferenceNumbersReferenceNumberTypePaxosFiatWithdrawalID                          PaymentOrderReferenceNumbersReferenceNumberType = "paxos_fiat_withdrawal_id"
	PaymentOrderReferenceNumbersReferenceNumberTypePaxosGroupID                                   PaymentOrderReferenceNumbersReferenceNumberType = "paxos_group_id"
	...
	PaymentOrderReferenceNumbersReferenceNumberTypeTurnkeyActivityID                              PaymentOrderReferenceNumbersReferenceNumberType = "turnkey_activity_id"
	PaymentOrderReferenceNumbersReferenceNumberTypeTurnkeySendTransactionStatusID                 PaymentOrderReferenceNumbersReferenceNumberType = "turnkey_send_transaction_status_id"
	...
	PaymentOrderReferenceNumbersReferenceNumberTypeWiseTransferID                                 PaymentOrderReferenceNumbersReferenceNumberType = "wise_transfer_id"
)

func (r PaymentOrderReferenceNumbersReferenceNumberType) IsKnown() bool {
	switch r {
	case PaymentOrderReferenceNumbersReferenceNumberTypeACHOriginalTraceNumber, PaymentOrderReferenceNumbersReferenceNumberTypeACHTraceNumber, PaymentOrderReferenceNumbersReferenceNumberTypeBankprovPaymentActivityDate, PaymentOrderReferenceNumbersReferenceNumberTypeBankprovPaymentID, PaymentOrderReferenceNumbersReferenceNumberTypeBlockchainTransactionHash, PaymentOrderReferenceNumbersReferenceNumberTypeBlockchainTransactionIndex, PaymentOrderReferenceNumbersReferenceNumberTypeBnkDevPrenotificationID, PaymentOrderReferenceNumbersReferenceNumberTypeBnkDevTransferID, PaymentOrderReferenceNumbersReferenceNumberTypeBnyMellonTransactionReferenceNumber, PaymentOrderReferenceNumbersReferenceNumberTypeBofaEndToEndID, PaymentOrderReferenceNumbersReferenceNumberTypeBofaNeftClearingReferenceNumber, PaymentOrderReferenceNumbersReferenceNumberTypeBofaTransactionID, PaymentOrderReferenceNumbersReferenceNumberTypeCheckNumber, PaymentOrderReferenceNumbersReferenceNumberTypeChipsUid, PaymentOrderReferenceNumbersReferenceNumberTypeCitibankReferenceNumber, PaymentOrderReferenceNumbersReferenceNumberTypeCitibankWorldlinkClearingSystemReferenceNumber, PaymentOrderReferenceNumbersReferenceNumberTypeColumnFxQuoteID, PaymentOrderReferenceNumbersReferenceNumberTypeColumnReversalPairTransferID, PaymentOrderReferenceNumbersReferenceNumberTypeColumnTransferID, PaymentOrderReferenceNumbersReferenceNumberTypeCrossRiverCardTraceNumber, PaymentOrderReferenceNumbersReferenceNumberTypeCrossRiverCoreTransactionID, PaymentOrderReferenceNumbersReferenceNumberTypeCrossRiverFedBatchID, PaymentOrderReferenceNumbersReferenceNumberTypeCrossRiverPaymentID, PaymentOrderReferenceNumbersReferenceNumberTypeCrossRiverRetrievalReferenceID, PaymentOrderReferenceNumbersReferenceNumberTypeCrossRiverServiceMessage, PaymentOrderReferenceNumbersReferenceNumberTypeCrossRiverTransactionID, PaymentOrderReferenceNumbersReferenceNumberTypeCurrencycloudConversionID, PaymentOrderReferenceNumbersReferenceNumberTypeCurrencycloudPaymentID, PaymentOrderReferenceNumbersReferenceNumberTypeEftTraceNumber, PaymentOrderReferenceNumbersReferenceNumberTypeEvolveCoreBatch, PaymentOrderReferenceNumbersReferenceNumberTypeEvolveCoreFileKey, PaymentOrderReferenceNumbersReferenceNumberTypeEvolveCoreSeq, PaymentOrderReferenceNumbersReferenceNumberTypeEvolveTransactionID, PaymentOrderReferenceNumbersReferenceNumberTypeFakeVendorPaymentID, PaymentOrderReferenceNumbersReferenceNumberTypeFedwireImad, PaymentOrderReferenceNumbersReferenceNumberTypeFedwireOmad, PaymentOrderReferenceNumbersReferenceNumberTypeFirstRepublicInternalID, PaymentOrderReferenceNumbersReferenceNumberTypeGoldmanSachsCollectionRequestID, PaymentOrderReferenceNumbersReferenceNumberTypeGoldmanSachsEndToEndID, PaymentOrderReferenceNumbersReferenceNumberTypeGoldmanSachsPaymentRequestID, PaymentOrderReferenceNumbersReferenceNumberTypeGoldmanSachsRequestID, PaymentOrderReferenceNumbersReferenceNumberTypeGoldmanSachsUniquePaymentID, PaymentOrderReferenceNumbersReferenceNumberTypeJpmcCcn, PaymentOrderReferenceNumbersReferenceNumberTypeJpmcClearingSystemReference, PaymentOrderReferenceNumbersReferenceNumberTypeJpmcCustomerReferenceID, PaymentOrderReferenceNumbersReferenceNumberTypeJpmcEndToEndID, PaymentOrderReferenceNumbersReferenceNumberTypeJpmcFirmRootID, PaymentOrderReferenceNumbersReferenceNumberTypeJpmcFxTrnID, PaymentOrderReferenceNumbersReferenceNumberTypeJpmcP3ID, PaymentOrderReferenceNumbersReferenceNumberTypeJpmcPaymentBatchID, PaymentOrderReferenceNumbersReferenceNumberTypeJpmcPaymentInformationID, PaymentOrderReferenceNumbersReferenceNumberTypeJpmcPaymentReturnedDatetime, PaymentOrderReferenceNumbersReferenceNumberTypeJpmcTransactionReferenceNumber, PaymentOrderReferenceNumbersReferenceNumberTypeLobCheckID, PaymentOrderReferenceNumbersReferenceNumberTypeOther, PaymentOrderReferenceNumbersReferenceNumberTypePartialSwiftMir, PaymentOrderReferenceNumbersReferenceNumberTypePaxosCryptoWithdrawalID, PaymentOrderReferenceNumbersReferenceNumberTypePaxosFiatWithdrawalID, PaymentOrderReferenceNumbersReferenceNumberTypePaxosGroupID, PaymentOrderReferenceNumbersReferenceNumberTypePaxosOrchestrationID, PaymentOrderReferenceNumbersReferenceNumberTypePaxosTransferID, PaymentOrderReferenceNumbersReferenceNumberTypePncClearingReference, PaymentOrderReferenceNumbersReferenceNumberTypePncInstructionID, PaymentOrderReferenceNumbersReferenceNumberTypePncMultipaymentID, PaymentOrderReferenceNumbersReferenceNumberTypePncPaymentTraceID, PaymentOrderReferenceNumbersReferenceNumberTypePncRequestForPaymentID, PaymentOrderReferenceNumbersReferenceNumberTypePncTransactionReferenceNumber, PaymentOrderReferenceNumbersReferenceNumberTypeRbcWireReferenceID, PaymentOrderReferenceNumbersReferenceNumberTypeRtpInstructionID, PaymentOrderReferenceNumbersReferenceNumberTypeSilvergatePaymentID, PaymentOrderReferenceNumbersReferenceNumberTypeSvbEndToEndID, PaymentOrderReferenceNumbersReferenceNumberTypeSvbPaymentID, PaymentOrderReferenceNumbersReferenceNumberTypeSwiftMir, PaymentOrderReferenceNumbersReferenceNumberTypeSwiftUetr, PaymentOrderReferenceNumbersReferenceNumberTypeTurnkeyActivityID, PaymentOrderReferenceNumbersReferenceNumberTypeTurnkeySendTransactionStatusID, PaymentOrderReferenceNumbersReferenceNumberTypeUmbProductPartnerAccountNumber, PaymentOrderReferenceNumbersReferenceNumberTypeUsbankPaymentApplicationReferenceID, PaymentOrderReferenceNumbersReferenceNumberTypeUsbankPaymentID, PaymentOrderReferenceNumbersReferenceNumberTypeUsbankPendingRtpPaymentID, PaymentOrderReferenceNumbersReferenceNumberTypeUsbankPostedRtpPaymentID, PaymentOrderReferenceNumbersReferenceNumberTypeWellsFargoEndToEndID, PaymentOrderReferenceNumbersReferenceNumberTypeWellsFargoPaymentID, PaymentOrderReferenceNumbersReferenceNumberTypeWellsFargoTraceNumber, PaymentOrderReferenceNumbersReferenceNumberTypeWellsFargoUetr, PaymentOrderReferenceNumbersReferenceNumberTypeWesternAlliancePaymentID, PaymentOrderReferenceNumbersReferenceNumberTypeWesternAllianceTransactionID, PaymentOrderReferenceNumbersReferenceNumberTypeWesternAllianceWireConfirmationNumber, PaymentOrderReferenceNumbersReferenceNumberTypeWiseTransferID:
		return true
	}
	return false
}
...
type PaymentOrderUltimateOriginatingAccount struct {
	...
	AccountDetails interface{} `json:"account_details" api:"required"`
	...
	RoutingDetails interface{} `json:"routing_details" api:"required"`
	UpdatedAt      time.Time   `json:"updated_at" api:"required" format:"date-time"`
	...
	AccountType PaymentOrderUltimateOriginatingAccountAccountType `json:"account_type" api:"nullable"`
	...
	ContraLedgerAccountID string `json:"contra_ledger_account_id" api:"nullable" format:"uuid"`
	// The ID of a credit normal ledger account. When money enters the virtual account,
	// this ledger account will be credited. Must be accompanied by a
	// debit_ledger_account_id if present.
	CreditLedgerAccountID string `json:"credit_ledger_account_id" api:"nullable" format:"uuid"`
	...
	LegalEntityID string `json:"legal_entity_id" api:"nullable" format:"uuid"`
	// The parent InternalAccount of this account.
	ParentAccountID string `json:"parent_account_id" api:"nullable" format:"uuid"`
...
const (
	...
	PaymentOrderSubtypeArbitrum                    PaymentOrderSubtype = "arbitrum"
	...
	PaymentOrderSubtypeDigital                     PaymentOrderSubtype = "digital"
	PaymentOrderSubtypeDkNets                      PaymentOrderSubtype = "dk_nets"
	PaymentOrderSubtypeEft                         PaymentOrderSubtype = "eft"
	PaymentOrderSubtypeEthereum                    PaymentOrderSubtype = "ethereum"
...
const (
	...
	PaymentOrderTypeStablecoin  PaymentOrderType = "stablecoin"
...
func (r PaymentOrderType) IsKnown() bool {
	switch r {
	case PaymentOrderTypeACH, PaymentOrderTypeAuBecs, PaymentOrderTypeBacs, PaymentOrderTypeBook, PaymentOrderTypeCard, PaymentOrderTypeChats, PaymentOrderTypeCheck, PaymentOrderTypeCrossBorder, PaymentOrderTypeDkNets, PaymentOrderTypeEft, PaymentOrderTypeGBFps, PaymentOrderTypeMasav, PaymentOrderTypeMxCcen, PaymentOrderTypeNeft, PaymentOrderTypeNics, PaymentOrderTypeNzBecs, PaymentOrderTypePlElixir, PaymentOrderTypeRtp, PaymentOrderTypeSeBankgirot, PaymentOrderTypeSepa, PaymentOrderTypeSgGiro, PaymentOrderTypeSic, PaymentOrderTypeStablecoin, PaymentOrderTypeWire, PaymentOrderTypeZengin:
		return true
	}
	return false
}
...
type PaymentOrderNewParams struct {
	...
	// `debit` pulls money from someone else's account to your own. Note that wire,
	// rtp, and check payments will always be `credit`.
	Direction param.Field[PaymentOrderNewParamsDirection] `json:"direction" api:"required"`
	// The ID of one of your organization's internal accounts.
...
const (
	PaymentOrderNewParamsReceivingAccountAccountDetailsAccountNumberTypeArbitrumAddress PaymentOrderNewParamsReceivingAccountAccountDetailsAccountNumberType = "arbitrum_address"
	...
	PaymentOrderNewParamsReceivingAccountAccountDetailsAccountNumberTypeCardToken       PaymentOrderNewParamsReceivingAccountAccountDetailsAccountNumberType = "card_token"
...
type PaymentOrderNewParamsReceivingAccountRoutingDetail struct {
	...
	RoutingNumberType param.Field[PaymentOrderNewParamsReceivingAccountRoutingDetailsRoutingNumberType] `json:"routing_number_type" api:"required"`
...
const (
	...
	PaymentOrderNewParamsReceivingAccountRoutingDetailsPaymentTypeStablecoin  PaymentOrderNewParamsReceivingAccountRoutingDetailsPaymentType = "stablecoin"
...
const (
	PaymentOrderUpdateParamsReceivingAccountAccountDetailsAccountNumberTypeArbitrumAddress PaymentOrderUpdateParamsReceivingAccountAccountDetailsAccountNumberType = "arbitrum_address"
	...
	PaymentOrderUpdateParamsReceivingAccountAccountDetailsAccountNumberTypeCardToken       PaymentOrderUpdateParamsReceivingAccountAccountDetailsAccountNumberType = "card_token"
...
type PaymentOrderUpdateParamsReceivingAccountRoutingDetail struct {
	RoutingNumber     param.Field[string]                                                                  `json:"routing_number" api:"required"`
	RoutingNumberType param.Field[PaymentOrderUpdateParamsReceivingAccountRoutingDetailsRoutingNumberType] `json:"routing_number_type" api:"required"`
...
const (
	...
	PaymentOrderUpdateParamsReceivingAccountRoutingDetailsPaymentTypeStablecoin  PaymentOrderUpdateParamsReceivingAccountRoutingDetailsPaymentType = "stablecoin"
...
const (
	...
	PaymentOrderListParamsTypeStablecoin  PaymentOrderListParamsType = "stablecoin"
...
func (r PaymentOrderListParamsType) IsKnown() bool {
	switch r {
	case PaymentOrderListParamsTypeACH, PaymentOrderListParamsTypeAuBecs, PaymentOrderListParamsTypeBacs, PaymentOrderListParamsTypeBook, PaymentOrderListParamsTypeCard, PaymentOrderListParamsTypeChats, PaymentOrderListParamsTypeCheck, PaymentOrderListParamsTypeCrossBorder, PaymentOrderListParamsTypeDkNets, PaymentOrderListParamsTypeEft, PaymentOrderListParamsTypeGBFps, PaymentOrderListParamsTypeMasav, PaymentOrderListParamsTypeMxCcen, PaymentOrderListParamsTypeNeft, PaymentOrderListParamsTypeNics, PaymentOrderListParamsTypeNzBecs, PaymentOrderListParamsTypePlElixir, PaymentOrderListParamsTypeRtp, PaymentOrderListParamsTypeSeBankgirot, PaymentOrderListParamsTypeSepa, PaymentOrderListParamsTypeSgGiro, PaymentOrderListParamsTypeSic, PaymentOrderListParamsTypeStablecoin, PaymentOrderListParamsTypeWire, PaymentOrderListParamsTypeZengin:
		return true
	}
	return false
}
...
type PaymentOrderNewAsyncParams struct {
	...
	// `debit` pulls money from someone else's account to your own. Note that wire,
	// rtp, and check payments will always be `credit`.
	Direction param.Field[PaymentOrderNewAsyncParamsDirection] `json:"direction" api:"required"`
	// The ID of one of your organization's internal accounts.
	OriginatingAccountID param.Field[string] `json:"originating_account_id" api:"required" format:"uuid"`
	// One of `ach`, `se_bankgirot`, `eft`, `wire`, `check`, `book`, `rtp`, `sepa`,
	// `bacs`, `au_becs`, `neft`, `nics`, `nz_national_clearing_code`, `sic`, `zengin`.
...
const (
	PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberTypeArbitrumAddress PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberType = "arbitrum_address"
	...
	PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberTypeCardToken       PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberType = "card_token"
...
func (r PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberType) IsKnown() bool {
	switch r {
	case PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberTypeArbitrumAddress, PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberTypeAuNumber, PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberTypeBaseAddress, PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberTypeCardToken, PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberTypeClabe, PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberTypeEthereumAddress, PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberTypeHkNumber, PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberTypeIban, PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberTypeIDNumber, PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberTypeNzNumber, PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberTypeOther, PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberTypePan, PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberTypePolygonAddress, PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberTypeSgNumber, PaymentOrderNewAsyncParamsReceivingAccountAccountDetailsAccountNumberTypeSolanaAddress:
		return true
	}
	return false
}
...
type PaymentOrderNewAsyncParamsReceivingAccountRoutingDetail struct {
	RoutingNumber     param.Field[string]                                                                    `json:"routing_number" api:"required"`
	RoutingNumberType param.Field[PaymentOrderNewAsyncParamsReceivingAccountRoutingDetailsRoutingNumberType] `json:"routing_number_type" api:"required"`
...
const (
	...
	PaymentOrderNewAsyncParamsReceivingAccountRoutingDetailsPaymentTypeStablecoin  PaymentOrderNewAsyncParamsReceivingAccountRoutingDetailsPaymentType = "stablecoin"
